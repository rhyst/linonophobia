...
function passNodesToWASM() {
  // TODO: clear bufs otherwise repeated calls cause memory leak
    const v = (v1, v2) => {
        if (!v2) {
    ...
    ccallArrays(
        "setFloats",
        null,
        ["array", "array", "array", "array", "array", "array", "array", "array", "number"],
        [
            v("position", "x"),
            v("position", "y"),
            v("velocity", "x"),
...
function doPhysics() {
    if (useWasm) {
        if (wasmReady) {
            setTimeout(cPhysics, 0);
        } else {
            setTimeout(doPhysics, 100);
        }
    } else {
        var delta = 0;
...
function cPhysics() {
    ccall("simulate", null, [], []);
    let id = ccallArrays("getIDs", "array", [], [], { heapOut: "HEAP32", returnArraySize: nodes.length });
    let x = ccallArrays("getPositionXs", "array", [], [], { heapOut: "HEAPF32", returnArraySize: nodes.length });
    let y = ccallArrays("getPositionYs", "array", [], [], { heapOut: "HEAPF32", returnArraySize: nodes.length });
    let vx = ccallArrays("getVelocityXs", "array", [], [], { heapOut: "HEAPF32", returnArraySize: nodes.length });
    let vy = ccallArrays("getVelocityXs", "array", [], [], { heapOut: "HEAPF32", returnArraySize: nodes.length });
    let fx = ccallArrays("getForceXs", "array", [], [], { heapOut: "HEAPF32", returnArraySize: nodes.length });
    let fy = ccallArrays("getForceYs", "array", [], [], { heapOut: "HEAPF32", returnArraySize: nodes.length });
    let f = ccallArrays("getFixeds", "array", [], [], { heapOut: "HEAP32", returnArraySize: nodes.length });
    let g = ccallArrays("getGrabbeds", "array", [], [], { heapOut: "HEAP32", returnArraySize: nodes.length });
    let cns = ccallArrays("getConnectedNodesSize", "array", [], [], {
        heapOut: "HEAP32",
        returnArraySize: nodes.length
    });
    let cn = [];
    let i = 0;
    cns.forEach(s => {
        cn.push(ccallArrays("getConnectedNodes", "array", ["number"], [i], { heapOut: "HEAP32", returnArraySize: s }));
        i++;
    });
    nodes = [];
    for (let j = 0; j < id.length; j++) {
        nodes.push(new Node(x[j], y[j], vx[j], vy[j], fx[j], fy[j], f[j] === 1, cn[j], id[j]));
    }
    elapsedTimeSumAverage = ccall("getElapsedTimeAverage", "number", [], []);
